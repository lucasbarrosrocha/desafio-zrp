...
- `lib/domain` — entities (`Episode`, `EpisodesPage`, `EpisodeDetail`, `CharacterSummary`, `CharacterDetail`) and the `EpisodeRepository`/`CharacterRepository` ports (mirroring the backend's own split — one port per upstream resource).
- `lib/data` — `HttpEpisodeRepository`/`HttpCharacterRepository`, the only layer that talks to the backend (via `package:http`).
- `lib/presentation` — screens, widgets, and Riverpod providers. `presentation` never calls the HTTP client directly, only the `domain` repository interfaces.
...
Tapping an episode card pushes `EpisodeDetailScreen` (`Navigator.push`), which fetches `GET /episodes/:id` through `episodeDetailProvider`, a `FutureProvider.family<EpisodeDetail, int>` — this one **is** `autoDispose`, unlike the list's own providers, since a detail screen's data has no reason to stay cached once it's popped off the stack. The screen renders the episode's fields plus its characters (avatar, name, a **View details** button), with its own loading/error/not-found (404) states. Back navigation is the platform's default `AppBar` back button; no extra code preserves the list's search/page, because Flutter never disposes the list screen's `State` (and therefore never disposes its providers) while the detail screen sits on top of it on the navigation stack.

**View details** opens `CharacterDetailBottomSheet` (`showModalBottomSheet`), fetching `GET /characters/:id` through `characterDetailProvider` — also `autoDispose`, for the same reason as the episode detail provider. Unlike the web modal, dismissing the sheet and reopening it *does* refetch: `showModalBottomSheet` pops its route entirely on close (not just hiding it, the way the web `Dialog`'s underlying React component stays mounted the whole time), tearing down the widget watching the provider along with it. The sheet's avatar (and the one on the character list) is `CharacterAvatar` — `Image.network` with an `errorBuilder`, not `CircleAvatar.backgroundImage` with `onBackgroundImageError`: the latter only marks the framework's own error report *silent* (it's still reported), while `errorBuilder` suppresses the report outright. The difference is invisible in a plain widget test (a silent report doesn't fail one) but not under `integration_test`, which fails a test on any reported `FlutterError` regardless of the silent flag — caught by a real, transient network hiccup on the emulator during this phase's own testing, not by either automated suite.
...
- **App: a "handled" image error can still fail an `integration_test`**. `CircleAvatar`'s `onBackgroundImageError` only marks the framework's own `FlutterError` report *silent* — it's still reported — while `Image`'s `errorBuilder` suppresses the report outright (confirmed in the Flutter SDK source: `Image` only sets `reportErrors: false` when `errorBuilder` is provided). A silent report doesn't fail a plain widget test, so this was invisible until a real, transient network hiccup on the Android emulator made an avatar image genuinely fail to load during this phase's own `integration_test` run, which does fail on any reported `FlutterError` regardless of the silent flag. `CharacterAvatar` (`app/lib/presentation/episodes/widgets/character_avatar.dart`) now uses `Image.network` + `errorBuilder` instead, for every avatar in the app.
...
_(This is the full set of endpoints for this challenge.)_
